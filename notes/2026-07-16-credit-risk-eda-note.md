...
오늘은 단순히 R 문법을 익히는 데 그치지 않고, 실제 신용위험 데이터를 처음 받았을 때 어떤 순서로 확인하고 해석해야 하는지 연습했다.

- 목표변수 `SeriousDlqin2yrs`의 의미 이해
- 전체 고객 수, 심각한 연체 고객 수, 연체율 계산
- 변수별 결측값 개수와 비율 확인
- `across()`를 이용해 여러 열에 같은 함수 적용
- 연령대별 심각한 연체율 비교
- `MonthlyIncome`의 분포와 극단값 조사
- 소득 구간을 직접 설계하고 그룹별 연체율 비교
